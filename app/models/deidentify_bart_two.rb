class DeidentifyBartTwo < ActiveRecord::Base
  ...
  def self.fake_person(person_id)
    self.transaction do
      ...
      #added the following for de-identification
      ...
      connection.execute("UPDATE #{target_db_name}.orders 
      SET patient_id = #{new_person_id} WHERE patient_id = #{person_id};")
  ...
  def self.fake_address(new_person_id)
    (PersonAddress.where(:person_id => new_person_id) || []).each do |address|
      PersonAddress.transaction do
        ...
        unless address.county_district.blank?
          address.state_province = Faker::Address.secondary_address
        end
                
        unless address.neighborhood_cell.blank?
          address.neighborhood_cell = Faker::Address.street_name
        end
  ...
  def self.fake_person_attribute(new_person_id)
    (PersonAttribute.where(:person_id => new_person_id) || []).each do |attribute|
      PersonAttribute.transaction do
        if attribute.person_attribute_type_id == 12 #cellphone number
            unless attribute.value == 'Unknown'
              attribute.value = Faker::PhoneNumber.cell_phone
            end
        elsif attribute.person_attribute_type_id == 14 #home phone number
            unless attribute.value == 'Unknown'
  ...
  def self.fake_patient_identifier(new_person_id)
    (PatientIdentifier.where(:patient_id => new_person_id) || []).each do |identifier|
      PatientIdentifier.transaction do
        unless identifier.identifier == 'Unknown'
            identifier.identifier = Faker::Number.number(12)
        end
